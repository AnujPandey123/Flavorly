...
const Recipes = () => {
  ...
  const getRecipes = () => {
    fetch("https://flavorly-backend.onrender.com/auth/recipe", {
      method: "GET",
      headers: {
    ...
    try {
      ...
      if (window.confirm("Are you sure you want to delete this recipe?")) {
        ...
        const response = await fetch(
          `https://flavorly-backend.onrender.com/auth/recipe/${recipeId}`,
          {
            method: "DELETE",
          }
        );
  ...
  const handleAddToFavorites = async (recipeId) => {
    try {
      ...
      const response = await fetch(
        `https://flavorly-backend.onrender.com/auth/likedRecipes/${recipeId}`,
        {
          method: "POST",
        }
      );
    ...
    try {
      if (e.target.value) {
        let Searchedrecipes = await fetch(
          `https://flavorly-backend.onrender.com/auth/searchRecipes/${e.target.value}`,
          {
            method: "GET",
            headers: {
